...
export default function ClientRequestsPage() {
  ...
  const filteredRequests = sortedRequests.filter(req => {
    ...
    return req.status?.toUpperCase() === selectedFilter;
  });

  const getEmptyStateText = (filter) => {
    switch (filter) {
      case 'PENDING':
    ...
    <div className="max-w-6xl w-full mx-auto p-4 sm:p-6 lg:p-8">
      ...
      {requests.length > 0 && (
        <div className="flex flex-wrap justify-center gap-2 mb-8">
          {[
            ...
            { label: 'Quote Received', value: 'QUOTE_SENT' },
            ...
            <button
              ...
              className={clsx(
                "px-5 py-2.5 rounded-full text-base font-bold transition-all duration-200 cursor-pointer",
      ...
      {requests.length === 0 ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 border-dashed p-12 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-gray-500 max-w-sm">Try changing your filters to see other requests.</p>
        </div>
      ) : (
          ...
          {filteredRequests.map(req => {
            ...
            return (
              <div
                key={req.id}
                className={clsx(
                  "rounded-2xl border shadow-sm p-6 flex flex-col sm:flex-row sm:items-center gap-6 transition-all",
